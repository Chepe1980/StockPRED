...
if not PLOTLY_AVAILABLE or not XGB_AVAILABLE:
    st.warning("⚠️ Some required packages are missing. Please install them to use all features.")
    st.markdown("""
    <div class="warning-box">
    Run the following commands to install missing packages:<br>
    <code>pip install plotly xgboost</code>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
        ...
        try:
            ...
            for j in range(1, 31):
                ...
                if lag_col in new_row.columns:
                    if j == 1:
                        new_row[lag_col] = current_data['Close'].iloc[-1]
                    else:
                        prev_lag = f'lag_{j-1}'
                        if prev_lag in current_data.columns:
                            new_row[lag_col] = current_data[prev_lag].iloc[-1]
                        else:
                            new_row[lag_col] = current_data['Close'].iloc[-1]
            
            current_data = pd.concat([current_data, new_row], ignore_index=True)
        except Exception as e:
            st.warning(f"Prediction stopped at day {i+1} due to error: {e}")
            break
...
if df is not None and not df.empty:
    ...
    with col2:
        if len(df) > 252:
            price_change = ((df['Close'].iloc[-1] - df['Close'].iloc[-252]) / df['Close'].iloc[-252] * 100)
        ...
        with st.spinner("Training XGBoost model... This may take a few moments..."):
            try:
                ...
                if len(df_features) < 100:
                    st.warning("Not enough data for reliable predictions. Please select a longer time period.")
                else:
                    # Split data
                    ...
                    if show_model_details:
                        ...
                        importance_df = pd.DataFrame({
                            'feature': feature_cols[:min(30, len(feature_cols))],
                            'importance': model.feature_importances_[:min(30, len(feature_cols))]
                        ...
                        fig_imp.update_layout(title='Top Feature Importance',
                    ...
                    with st.spinner("Generating future predictions..."):
                        ...
                        future_prices = predict_future(model, last_data, feature_cols, days_ahead=730)
                        ...
                        if len(future_prices) > 0:
                            ...
                            std_dev = np.std(y_test - y_pred) if len(y_test) > 0 else rmse
                            ...
                            fig_future.add_trace(go.Scatter(x=hist_data.index, 
                                                            y=hist_data['Close'],
                                                            mode='lines', 
                            ...
                            st.error("Failed to generate predictions. Please try again with different settings.")
